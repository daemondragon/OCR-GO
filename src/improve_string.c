...
void add_punctuation(char *string, W_list *list)
{
    infos *actual = list->first;
    size_t global_height = 0;
    if (actual)
        global_height = actual->height;

    while (*string && actual)
    {
        if (actual->height * 3 < global_height)
            *string = '.';
...
void improve_string(char *string, W_list *list)
{
    if (!string)
        return;

    if (list)
        add_punctuation(string, list);
